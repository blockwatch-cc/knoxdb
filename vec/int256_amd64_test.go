...
func TestMatchInt256EqualAVX2(T *testing.T) {
	...
	for _, c := range Int256EqualCases {
		...
		bits = bits[:l]
		cnt := matchInt256EqualAVX2(c.slice.Optimize(), c.match, bits)
...
func TestMatchInt256EqualAVX512(T *testing.T) {
    ...
    for _, c := range Int256EqualCases {
        ...
        bits = bits[:l]
        cnt := matchInt256EqualAVX512(c.slice.Optimize(), c.match, bits)
        if got, want := len(bits), len(c.result); got != want {
            T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
        }
        if got, want := cnt, c.count; got != want {
...
func TestMatchInt256NotEqualAVX2(T *testing.T) {
	...
	for _, c := range Int256NotEqualCases {
		...
		bits = bits[:l]
		cnt := matchInt256NotEqualAVX2(c.slice.Optimize(), c.match, bits)
		if got, want := len(bits), len(c.result); got != want {
			T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
		}
		if got, want := cnt, c.count; got != want {
...
func TestMatchInt256NotEqualAVX512(T *testing.T) {
    ...
    for _, c := range Int256NotEqualCases {
        ...
        bits = bits[:l]
        cnt := matchInt256NotEqualAVX512(c.slice.Optimize(), c.match, bits)
        if got, want := len(bits), len(c.result); got != want {
            T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
        }
        if got, want := cnt, c.count; got != want {
...
func TestMatchInt256LessAVX2(T *testing.T) {
	...
	for _, c := range Int256LessCases {
		...
		bits = bits[:l]
		cnt := matchInt256LessThanAVX2(c.slice.Optimize(), c.match, bits)
		if got, want := len(bits), len(c.result); got != want {
			T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
		}
		if got, want := cnt, c.count; got != want {
...
func TestMatchInt256LessAVX512(T *testing.T) {
    ...
    for _, c := range Int256LessCases {
        ...
        bits = bits[:l]
        cnt := matchInt256LessThanAVX512(c.slice.Optimize(), c.match, bits)
        if got, want := len(bits), len(c.result); got != want {
            T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
        }
        if got, want := cnt, c.count; got != want {
...
func TestMatchInt256LessEqualAVX2(T *testing.T) {
	...
	for _, c := range Int256LessEqualCases {
		...
		bits = bits[:l]
		cnt := matchInt256LessThanEqualAVX2(c.slice.Optimize(), c.match, bits)
		if got, want := len(bits), len(c.result); got != want {
			T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
		}
		if got, want := cnt, c.count; got != want {
...
func TestMatchInt256LessEqualAVX512(T *testing.T) {
    ...
    for _, c := range Int256LessEqualCases {
        ...
        bits = bits[:l]
        cnt := matchInt256LessThanEqualAVX512(c.slice.Optimize(), c.match, bits)
        if got, want := len(bits), len(c.result); got != want {
            T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
        }
        if got, want := cnt, c.count; got != want {
...
func TestMatchInt256GreaterAVX2(T *testing.T) {
	...
	for _, c := range Int256GreaterCases {
		...
		bits = bits[:l]
		cnt := matchInt256GreaterThanAVX2(c.slice.Optimize(), c.match, bits)
		if got, want := len(bits), len(c.result); got != want {
			T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
		}
		if got, want := cnt, c.count; got != want {
...
func TestMatchInt256GreaterAVX512(T *testing.T) {
    ...
    for _, c := range Int256GreaterCases {
        ...
        bits = bits[:l]
        cnt := matchInt256GreaterThanAVX512(c.slice.Optimize(), c.match, bits)
        if got, want := len(bits), len(c.result); got != want {
            T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
        }
        if got, want := cnt, c.count; got != want {
...
func TestMatchInt256GreaterEqualAVX2(T *testing.T) {
	...
	for _, c := range Int256GreaterEqualCases {
		...
		bits = bits[:l]
		cnt := matchInt256GreaterThanEqualAVX2(c.slice.Optimize(), c.match, bits)
		if got, want := len(bits), len(c.result); got != want {
			T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
		}
		if got, want := cnt, c.count; got != want {
...
func TestMatchInt256GreaterEqualAVX512(T *testing.T) {
    ...
    for _, c := range Int256GreaterEqualCases {
        ...
        bits = bits[:l]
        cnt := matchInt256GreaterThanEqualAVX512(c.slice.Optimize(), c.match, bits)
        if got, want := len(bits), len(c.result); got != want {
            T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
        }
        if got, want := cnt, c.count; got != want {
...
func TestMatchInt256BetweenAVX2(T *testing.T) {
	...
	for _, c := range Int256BetweenCases {
		...
		bits = bits[:l]
		cnt := matchInt256BetweenAVX2(c.slice.Optimize(), c.match, c.match2, bits)
		if got, want := len(bits), len(c.result); got != want {
			T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
		}
		if got, want := cnt, c.count; got != want {
...
func TestMatchInt256BetweenAVX512(T *testing.T) {
    ...
    for _, c := range Int256BetweenCases {
        ...
        bits = bits[:l]
        cnt := matchInt256BetweenAVX512(c.slice.Optimize(), c.match, c.match2, bits)
        if got, want := len(bits), len(c.result); got != want {
            T.Errorf("%s: unexpected result length %d, expected %d", c.name, got, want)
        }
        if got, want := cnt, c.count; got != want {
