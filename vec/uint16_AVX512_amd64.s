...
TEXT ·matchUint16EqualAVX512(SB), NOSPLIT, $0-64
...
// works for >= 64 uint16 (i.e. 128 bytes of data)
loop_big:
	VPCMPEQW	(SI), Z0, K1
...
//   K1-K7 = comparision results
//   CX = loop counter (counts 1/8 values or bytes writen to output slice, runs from neg. to zero)
TEXT ·matchUint16NotEqualAVX512(SB), NOSPLIT, $0-64
	MOVQ	src_base+0(FP), SI
	MOVQ	src_len+8(FP), BX
	MOVQ	bits_base+32(FP), DI
	XORQ	R9, R9
...
//   K1-K7 = comparision results
//   CX = loop counter (counts 1/8 values or bytes writen to output slice, runs from neg. to zero)
TEXT ·matchUint16LessThanAVX512(SB), NOSPLIT, $0-64
	MOVQ	src_base+0(FP), SI
	MOVQ	src_len+8(FP), BX
	MOVQ	bits_base+32(FP), DI
	XORQ	R9, R9
...
// works for >= 64 uint16 (i.e. 128 bytes of data)
loop_big:
	VPCMPUW	    $6, (SI), Z0, K1    // $6 means not less equal, but operands are switched
    
	VPCMPUW	    $6, 64(SI), Z0, K2 
    KSHIFTLQ    $32, K2, K2
    KORQ        K1, K2, K1
...
//   K1-K7 = comparision results
//   CX = loop counter (counts 1/8 values or bytes writen to output slice, runs from neg. to zero)
TEXT ·matchUint16LessThanEqualAVX512(SB), NOSPLIT, $0-64
	MOVQ	src_base+0(FP), SI
	MOVQ	src_len+8(FP), BX
	MOVQ	bits_base+32(FP), DI
	XORQ	R9, R9
...
// works for >= 64 uint16 (i.e. 128 bytes of data)
loop_big:
	VPCMPUW	    $5, (SI), Z0, K1      // $5 means not less than, but operands are switched
    
	VPCMPUW	    $5, 64(SI), Z0, K2 
    KSHIFTLQ    $32, K2, K2
    KORQ        K1, K2, K1
...
//   K1-K7 = comparision results
//   CX = loop counter (counts 1/8 values or bytes writen to output slice, runs from neg. to zero)
TEXT ·matchUint16GreaterThanAVX512(SB), NOSPLIT, $0-64
	MOVQ	src_base+0(FP), SI
	MOVQ	src_len+8(FP), BX
	MOVQ	bits_base+32(FP), DI
	XORQ	R9, R9
...
// works for >= 64 uint16 (i.e. 128 bytes of data)
loop_big:
	VPCMPUW	    $1, (SI), Z0, K1    // $1 means less than, but operands are switched
    
	VPCMPUW	    $1, 64(SI), Z0, K2 
    KSHIFTLQ    $32, K2, K2
    KORQ        K1, K2, K1
...
//   K1-K7 = comparision results
//   CX = loop counter (counts 1/8 values or bytes writen to output slice, runs from neg. to zero)
TEXT ·matchUint16GreaterThanEqualAVX512(SB), NOSPLIT, $0-64
	MOVQ	src_base+0(FP), SI
	MOVQ	src_len+8(FP), BX
	MOVQ	bits_base+32(FP), DI
	XORQ	R9, R9
...
// works for >= 64 uint16 (i.e. 128 bytes of data)
loop_big:
	VPCMPUW	    $2, (SI), Z0, K1    // $2 means less equal, but operands are switched
    
	VPCMPUW	    $2, 64(SI), Z0, K2 
    KSHIFTLQ    $32, K2, K2
    KORQ        K1, K2, K1
...
// func matchUint16BetweenAVX512(src []uint64, a, b uint64, bits []byte) int64
//
// input:
//   SI = src_base
//   DI = bits_base
