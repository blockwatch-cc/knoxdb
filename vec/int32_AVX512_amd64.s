...
TEXT ·matchInt32EqualAVX512(SB), NOSPLIT, $0-64
...
// works for >= 64 int32 (i.e. 256 bytes of data)
loop_big:
	VPCMPEQD	(SI), Z0, K1
...
//   K1-K7 = comparision results
//   CX = loop counter (counts 1/8 values or bytes writen to output slice, runs from neg. to zero)
TEXT ·matchInt32NotEqualAVX512(SB), NOSPLIT, $0-64
	MOVQ	src_base+0(FP), SI
	MOVQ	src_len+8(FP), BX
	MOVQ	bits_base+32(FP), DI
	XORQ	R9, R9
...
//   K1-K7 = comparision results
//   CX = loop counter (counts 1/8 values or bytes writen to output slice, runs from neg. to zero)
TEXT ·matchInt32LessThanAVX512(SB), NOSPLIT, $0-64
	MOVQ	src_base+0(FP), SI
	MOVQ	src_len+8(FP), BX
	MOVQ	bits_base+32(FP), DI
	XORQ	R9, R9
...
// works for >= 64 int32 (i.e. 256 bytes of data)
loop_big:
	VPCMPD	    $6, (SI), Z0, K1    // $6 means not less equal, but operands are switched
    
	VPCMPD	    $6, 64(SI), Z0, K2 
    KSHIFTLQ    $16, K2, K2
    KORQ        K1, K2, K1

	VPCMPD	    $6, 128(SI), Z0, K3 
    KSHIFTLQ    $32, K3, K3
    KORQ        K1, K3, K1

	VPCMPD	    $6, 192(SI), Z0, K4 
    KSHIFTLQ    $48, K4, K4
    KORQ        K1, K4, K1
...
//   K1-K7 = comparision results
//   CX = loop counter (counts 1/8 values or bytes writen to output slice, runs from neg. to zero)
TEXT ·matchInt32LessThanEqualAVX512(SB), NOSPLIT, $0-64
	MOVQ	src_base+0(FP), SI
	MOVQ	src_len+8(FP), BX
	MOVQ	bits_base+32(FP), DI
	XORQ	R9, R9
...
// works for >= 64 int32 (i.e. 256 bytes of data)
loop_big:
	VPCMPD	    $5, (SI), Z0, K1      // $5 means not less than, but operands are switched
    
	VPCMPD	    $5, 64(SI), Z0, K2 
    KSHIFTLQ    $16, K2, K2
    KORQ        K1, K2, K1

	VPCMPD	    $5, 128(SI), Z0, K3 
    KSHIFTLQ    $32, K3, K3
    KORQ        K1, K3, K1

	VPCMPD	    $5, 192(SI), Z0, K4 
    KSHIFTLQ    $48, K4, K4
    KORQ        K1, K4, K1
...
//   K1-K7 = comparision results
//   CX = loop counter (counts 1/8 values or bytes writen to output slice, runs from neg. to zero)
TEXT ·matchInt32GreaterThanAVX512(SB), NOSPLIT, $0-64
	MOVQ	src_base+0(FP), SI
	MOVQ	src_len+8(FP), BX
	MOVQ	bits_base+32(FP), DI
	XORQ	R9, R9
...
// works for >= 64 int32 (i.e. 256 bytes of data)
loop_big:
	VPCMPD	    $1, (SI), Z0, K1    // $1 means less than, but operands are switched
    
	VPCMPD	    $1, 64(SI), Z0, K2 
    KSHIFTLQ    $16, K2, K2
    KORQ        K1, K2, K1

	VPCMPD	    $1, 128(SI), Z0, K3 
    KSHIFTLQ    $32, K3, K3
    KORQ        K1, K3, K1

	VPCMPD	    $1, 192(SI), Z0, K4 
    KSHIFTLQ    $48, K4, K4
    KORQ        K1, K4, K1
...
//   K1-K7 = comparision results
//   CX = loop counter (counts 1/8 values or bytes writen to output slice, runs from neg. to zero)
TEXT ·matchInt32GreaterThanEqualAVX512(SB), NOSPLIT, $0-64
	MOVQ	src_base+0(FP), SI
	MOVQ	src_len+8(FP), BX
	MOVQ	bits_base+32(FP), DI
	XORQ	R9, R9
...
// works for >= 64 int32 (i.e. 256 bytes of data)
loop_big:
	VPCMPD	    $2, (SI), Z0, K1    // $2 means less equal, but operands are switched
    
	VPCMPD	    $2, 64(SI), Z0, K2 
    KSHIFTLQ    $16, K2, K2
    KORQ        K1, K2, K1

	VPCMPD	    $2, 128(SI), Z0, K3 
    KSHIFTLQ    $32, K3, K3
    KORQ        K1, K3, K1

	VPCMPD	    $2, 192(SI), Z0, K4 
    KSHIFTLQ    $48, K4, K4
    KORQ        K1, K4, K1
...
// func matchInt32BetweenAVX512(src []uint64, a, b uint64, bits []byte) int64
//
// input:
//   SI = src_base
//   DI = bits_base
...
//   K1-K7 = comparision results
//   CX = loop counter (counts 1/8 values or bytes writen to output slice, runs from neg. to zero)
TEXT ·matchInt32BetweenAVX512(SB), NOSPLIT, $0-72
	MOVQ	src_base+0(FP), SI
	MOVQ	src_len+8(FP), BX
	MOVQ	bits_base+32(FP), DI
	XORQ	R9, R9
