...
func TestBitOrGenericFlag2(T *testing.T) {
	if !useAVX2 {
		T.SkipNow()
	}
	// calls use the function selector to do proper last byte masking!
	for _, sz := range bitsetSizes {
		...
		for _, pt := range bitsetPatterns {
			T.Run(f("%d_%x", sz, pt), func(t *testing.T) {
				...
				// same value, same slice
				any, all := bitsetOrGenericFlag2(dst, dst, sz)
				if pt == 0x80 && sz == 7 {
					if any {
						T.Errorf("dst===src: unexpected return value %v, expected false", any)
					}
				...
				// same value, other slice
				copy(dst, src)
				any, all = bitsetOrGenericFlag2(dst, src, sz)
				if pt == 0x80 && sz == 7 {
					if any {
						T.Errorf("dst==src: unexpected return value %v, expected false", any)
					}
				...
				// all zeros
				copy(dst, src)
				any, all = bitsetOrGenericFlag2(dst, zeros, sz)
				if pt == 0x80 && sz == 7 {
					if any {
						T.Errorf("zeros: unexpected return value %v, expected false", any)
					}
				...
				// all ones
				copy(dst, src)
				any, all = bitsetOrGenericFlag2(dst, ones, sz)
				if !any {
					T.Errorf("ones: unexpected return value %v, expected true", any)
				}
				if !all {
...
func TestBitOrAVX2Flag2(T *testing.T) {
	if !useAVX2 {
		T.SkipNow()
	}
	// calls use the function selector to do proper last byte masking!
	for _, sz := range bitsetSizes {
		...
		for _, pt := range bitsetPatterns {
			T.Run(f("%d_%x", sz, pt), func(t *testing.T) {
				...
				// same value, same slice
				any, all := bitsetOrAVX2Flag2(dst, dst, sz)
				if pt == 0x80 && sz == 7 {
					if any {
						T.Errorf("dst===src: unexpected return value %v, expected false", any)
					}
				...
				// same value, other slice
				copy(dst, src)
				any, all = bitsetOrAVX2Flag2(dst, src, sz)
				if pt == 0x80 && sz == 7 {
					if any {
						T.Errorf("dst==src: unexpected return value %v, expected false", any)
					}
				...
				// all zeros
				copy(dst, src)
				any, all = bitsetOrAVX2Flag2(dst, zeros, sz)
				if pt == 0x80 && sz == 7 {
					if any {
						T.Errorf("zeros: unexpected return value %v, expected false", any)
					}
				...
				// all ones
				copy(dst, src)
				any, all = bitsetOrAVX2Flag2(dst, ones, sz)
				if !any {
					T.Errorf("ones: unexpected return value %v, expected true", any)
				}
				if !all {
