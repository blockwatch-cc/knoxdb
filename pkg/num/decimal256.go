...
var ZeroDecimal256 = Decimal256{ZeroInt256, 0}
...
func NewDecimal256(val Int256, scale uint8) Decimal256 {
...
func (d Decimal256) Scale() uint8 {
	return d.scale
}

func (d Decimal256) Precision() uint8 {
	switch {
	case d.val.IsInt64():
		val := abs(d.val.Int64())
		for i := range pow10 {
...
func (d Decimal256) Quantize(scale uint8) Decimal256 {
	if scale == d.scale {
		return d
	}
...
func (d *Decimal256) SetScale(scale uint8) {
	d.scale = scale
}

func (d *Decimal256) SetInt64(value int64, scale uint8) error {
	if scale > MaxDecimal256Precision {
...
func (d *Decimal256) SetInt128(value Int128, scale uint8) error {
	if scale > MaxDecimal256Precision {
		return ErrScaleOverflow
	}
	d.scale = scale
...
func (d *Decimal256) SetInt256(value Int256, scale uint8) error {
	if scale > MaxDecimal256Precision {
		return ErrScaleOverflow
	}
	d.scale = scale
...
func (d *Decimal256) SetFloat64(value float64, scale uint8) error {
	if scale > MaxDecimal256Precision {
		return ErrScaleOverflow
	}
...
func (d Decimal256) String() string {
	...
	default:
		...
		b.Grow(int(MaxDecimal256Precision + 2))
		sign := 0
		if i[0] == '-' {
			b.WriteRune('-')
			sign = 1
...
func MustParseDecimal256(s string) Decimal256 {
...
func CompareDecimal256(a, b Decimal256) int {
	return a.Cmp(b)
}
