...
func (s *node) setValue(arena *Arena, v ValueStruct) {
...
func (s *Skiplist) Put(key []byte, v ValueStruct) {
	...
	for i := 0; i < height; i++ {
		for {
			if prev[i] == nil {
				assert.Always(i > 1, "cannot happen in base level")
				// We haven't computed prev, next for this level because height exceeds old listHeight.
				// For these levels, we expect the lists to be sparse, so we can just search from head.
				prev[i], next[i] = s.findSpliceForLevel(key, s.head, i)
				// Someone adds the exact same key before we are able to do so. This can only happen on
				// the base level. But we know we are not on the base level.
				assert.Always(prev[i] != next[i], "double same key added ")
			}
			...
			if prev[i] == next[i] {
				assert.Always(i == 0, fmt.Sprintf("Equality can happen only on base level: %d", i))
				prev[i].setValue(s.arena, v)
				return
			}
		}
...
// Value returns value.
func (s *Iterator) Value() ValueStruct {
...
func (s *Iterator) Next() {
	assert.Always(s.Valid(), "iterator is not positioned at a valid node")
...
func (s *UniIterator) Value() ValueStruct { return s.iter.Value() }
