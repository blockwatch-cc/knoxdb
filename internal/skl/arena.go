...
func (s *Arena) putNode(height int) uint32 {
	...
	n := s.n.Add(l)
	assert.Always(int(n) <= len(s.buf), fmt.Sprintf("Arena too small, toWrite:%d newTotal:%d limit:%d",
		l, n, len(s.buf)))
...
func (s *Arena) putKey(key []byte) uint32 {
	...
	assert.Always(len(key) == copy(s.buf[m:n], key), "copy contents of key to buffer")
	return m
}
