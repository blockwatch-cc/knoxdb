	...
	for _, gen := range tests.Generators {
		...
		tests := []TestStruct{
			...
			},
			{
				...
				comment:  "single NI should simplify to NE",
			},
			// TODO: better to have an anti-range filter type
			// {
			// 	name:      "NI(1,2,3) => LT(1) OR GT(3)",
			// 	input:     makeAndTree(makeNotInNode(f1, s(42, 43, 44))),
			// 	expected:  makeOrTree(makeLtNode(f1, v(42)), makeGtNode(f1, v(44))),
			// 	comment:   "range NI should simplify to OR",
			// 	skipTypes: []BlockType{BlockBool, BlockBytes, BlockFloat64, BlockFloat32},
			// },
			{
				...
				comment:   "multiple NI should simplify to single NI",
			...
			},
			{
				...
				comment:   "multiple NI with OR should simplify to intersection",
				skipTypes: []BlockType{BlockBool},
			},
			{
