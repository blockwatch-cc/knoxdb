...
// find the highest checkpoint across all containers to open and check wal integrity
func (e *Engine) maxCheckpoint() (lsn wal.LSN) {
	lsn = e.cat.Checkpoint()
	for _, v := range e.tables {
		lsn = max(lsn, v.State().Checkpoint)
	}
...
// // find the lowest checkpoint across all containers to start wal replay on startup
// func (e *Engine) minWalCheckpoint() (minLsn wal.LSN) {
// 	minLsn = e.cat.checkpoint
// 	for _, v := range e.tables {
// 		minLsn = min(minLsn, v.State().Checkpoint)
// 	}
// 	for _, v := range e.stores {
// 		minLsn = min(minLsn, v.State().Checkpoint)
// 	}
...
// func (e *Engine) getWalCheckpoint(entity uint64) wal.LSN {
// 	if entity == e.dbId {
...
// 			e.log.Debugf("Skip Record %s", rec)
// 			continue
// 		}
