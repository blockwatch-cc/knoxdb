...
// EmptyBlockMetadata occupies minimal storage and may be used for deleted schema fields.
var EmptyBlockMetadata = BlockMetadata{
	Type:     BlockBool,
	MinValue: false,
	MaxValue: false,
}

func (m BlockMetadata) IsValid() bool {
	return m.Type.IsValid() && m.MinValue != nil && m.MaxValue != nil
}

func (m BlockMetadata) IsDirty() bool {
	return m.dirty
}

func (m *BlockMetadata) SetDirty() {
	m.dirty = true
}

func NewBlockMetadata(b *block.Block, f *schema.Field) BlockMetadata {
	m := BlockMetadata{
		Type:  b.Type(),
		dirty: b.Len() > 0,
	}
...
func (m BlockMetadata) canUseBitsets() bool {
...
func (m BlockMetadata) HeapSize() int {
	sz := szBlockMetadata
	switch m.Type {
	case BlockInt64, BlockTime, BlockUint64, BlockFloat64:
		sz += 16
...
func (m BlockMetadata) Encode(buf *bytes.Buffer) error {
	// type encoding
	// 8                         7 6          5 4 3 2 1
...
func (m *BlockMetadata) Decode(buf *bytes.Buffer, version byte) error {
	typ := buf.Next(1)
