...
func makeSortedPackMetadataList(n int) PackMetadataList {
...
func makeUnsortedPackMetadataList(n int) PackMetadataList {
	// generate random values, strip duplicates and sort
	values := randUint64Slice(n, 1)
...
type packIndexTestCase struct {
	Name   string
	Info   *PackMetadata // used for Add
...
func buildPackHeader(key uint32, min, max uint64) *PackMetadata {
	return &PackMetadata{
		Key:     key,
		NValues: 1,
		Blocks: []BlockMetadata{
...
func buildPackHeaderInt(key int, min, max uint64) *PackMetadata {
	return buildPackHeader(uint32(key), min, max)
}

func buildPackMetadataList(items []packIndexTestListItem) PackMetadataList {
	packs := make(PackMetadataList, len(items))
	for i, v := range items {
...
func TestPackIndexBest(t *testing.T) {
	for _, c := range packIndexTestCases {
		idx := NewMetadataIndex(0, 1)
		for _, v := range buildPackMetadataList(c.List) {
			idx.AddOrUpdate(v)
		}
...
func BenchmarkPackIndexBestSorted(B *testing.B) {
	for _, n := range bestPackBenchmarkSizes {
		B.Run(fmt.Sprintf("%s", n.name), func(B *testing.B) {
			v2 := NewMetadataIndex(0, 1)
			for _, v := range makeSortedPackMetadataList(n.l) {
				v2.AddOrUpdate(v)
			}
...
func BenchmarkPackIndexBestUnsorted(B *testing.B) {
	for _, n := range bestPackBenchmarkSizes {
		B.Run(fmt.Sprintf("%s", n.name), func(B *testing.B) {
			v2 := NewMetadataIndex(0, 1)
			for _, v := range makeUnsortedPackMetadataList(n.l) {
				v2.AddOrUpdate(v)
			}
			max := v2.packs[v2.pos[len(v2.pos)-1]].Blocks[v2.pki].MaxValue.(uint64)
			B.ResetTimer()
