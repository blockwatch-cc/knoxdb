...
func BitmapAnd() {
    TEXT("bitmapOrAvx2", NOSPLIT|NOPTR, "func(data []uint16, other []uint16, buf []uint16) int")
    Pragma("noescape")
    Doc("bitmapOrAvx2 performs an And operation between data []uint16 and other []uint16. \n // Returns data with values and cardinality.\n")
...
func BitmapOr() {
    TEXT("bitmapAndAvx2", NOSPLIT|NOPTR, "func(data []uint16, other []uint16) int")
    Pragma("noescape")
    Doc("bitmapAndAvx2 performs an Or operation between data []uint16 and other []uint16. \n // Returns data with values and cardinality.\n")
    a := Mem{Base: Load(Param("data").Base(), GP64())}
    b := Mem{Base: Load(Param("other").Base(), GP64())}
...
func BitmapAndNot() {
    TEXT("bitmapAndNotAvx2", NOSPLIT|NOPTR, "func(data []uint16, other []uint16, buf []uint16) int")
    Pragma("noescape")
    Doc("bitmapAndNotAvx2 performs an AndNot operation between data []uint16 and other []uint16. \n // Returns data with values and cardinality.\n")
    a := Mem{Base: Load(Param("data").Base(), GP64())}
    b := Mem{Base: Load(Param("other").Base(), GP64())}
    c := Mem{Base: Load(Param("buf").Base(), GP64())}
