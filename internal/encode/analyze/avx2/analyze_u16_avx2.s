...
TEXT ·analyze_u16_avx2(SB), NOSPLIT, $0-24
...
first_loop:
    ...
    VPMINUW Y1, Y4, Y4          // Update min_vec
    VPMAXUW Y1, Y5, Y5          // Update max_vec
...
vector_loop:
    ...
    VPMINUW Y1, Y4, Y4     // Y4 = min(Y4, Y1), unsigned 16-bit min across 16 lanes
    VPMAXUW Y1, Y5, Y5     // Y5 = max(Y5, Y1), unsigned 16-bit max across 16 lanes
...
tail_loop:
    ...
    CMPW R12, AX
    JAE min_done           // Signed comparison: if R12 >= AX
    MOVW R12, AX           // Update AX
min_done:
    CMPW R12, DX
    JBE max_done           // Signed comparison: if R12 <= DX
