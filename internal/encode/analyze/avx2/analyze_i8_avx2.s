...
TEXT ·analyze_i8_avx2(SB), NOSPLIT, $0-24
    // Load arguments (Go calling convention)
    MOVQ vals+0(FP), R8    // R8 = pointer to vals array (int8_t*)
    MOVQ ctx+8(FP), DI     // DI = pointer to ctx struct (Context[int8])
...
first_loop:
    ...
    VPERM2I128 $0x3, Y6, Y1, Y2
    VPALIGNR $15, Y2, Y1, Y3
    VMOVDQA Y1, Y6
...
vector_loop:
    ...
    VPMINSB Y1, Y4, Y4     // Y4 = min(Y4, Y1), signed 8-bit min across 32 lanes
    VPMAXSB Y1, Y5, Y5     // Y5 = max(Y5, Y1), signed 8-bit max across 32 lanes

    // Create shifted vector
    VPERM2I128 $0x3, Y6, Y1, Y2 // Y2 = [Y6[127:0], Y1[255:128]]
    VPALIGNR $15, Y2, Y1, Y3 // Y3 = shifted, align right by 15 bytes (31x8 <
...
run_done:
    ...
    SUBB R13, R15          // R15 = vals[i] - last_prev (signed)
