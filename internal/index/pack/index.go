...
func (idx *Index) Create(ctx context.Context, t engine.TableEngine, s *schema.Schema, opts engine.IndexOptions) error {
	...
	idx.indexId = s.TaggedHash(types.HashTagIndex)
	idx.opts = opts
	...
	idx.log.Debugf("Creating pack index %s on %s with driver %s", name, t.Schema().Name(), opts.Driver)

	// create db if not passed in options
	if idx.db == nil {
		path := filepath.Join(e.RootPath(), name+".db")
		idx.log.Debugf("Creating pack index %q with opts %#v", path, opts)
...
func (idx *Index) Open(ctx context.Context, t engine.TableEngine, s *schema.Schema, opts engine.IndexOptions) error {
	...
	idx.log.Debugf("Opening pack index %s on %s with driver %s", name, t.Schema().Name(), opts.Driver)

	// open db if not passed in options
	if idx.db == nil {
		path := filepath.Join(e.RootPath(), name+".db")
		idx.log.Debugf("Opening pack index %q with opts %#v", path, opts)
