...
// AppendWire appends a new row of values from a wire protocol message.
// The caller must ensure the message matches the currrent package schema.
// When metadata columns exist, AppendWire writes metadata values from
// meta unless it is nil in which case zeros are written instead.
