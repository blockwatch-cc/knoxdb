...
func TestTombAppend(t *testing.T) {
	...
	require.Equal(t, uint64(1), tomb.Stones()[0].Xid)
	...
	require.Equal(t, uint64(1), tomb.Stones()[0].Xid)
	require.Equal(t, uint64(1), tomb.Stones()[1].Xid)
	require.Equal(t, uint64(2), tomb.Stones()[2].Xid)
...
func TestTombAbortActive(t *testing.T) {
	...
	require.Equal(t, 1, tomb.Len())
	require.Equal(t, uint64(2), tomb.Stones()[0].Xid)
	require.Equal(t, 1, tomb.RowIds().Count())
}
