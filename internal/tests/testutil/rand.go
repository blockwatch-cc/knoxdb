// Copyright (c) 20242-2026 Blockwatch Data Inc.
...
func RandIntsRange[T Signed](sz int, min, max T) []T {
	s := RandIntsn(sz, max-min)
	for i := range s {
		s[i] += min
	}
	return s
...
func RandIntRange[T Signed](min, max T) (T, T) {
...
func RandUintsRange[T Unsigned](sz int, min, max T) []T {
	s := RandUintsn(sz, max-min)
	for i := range s {
		s[i] += min
	}
	return s
...
func RandFloats[T Float](sz int) []T {
	s := make([]T, sz)
	for i := range sz {
		s[i] = T(RandFloat64())
	}
...
func RandFloatsn[T Float](sz int, max T) []T {
	s := make([]T, sz)
	for i := range sz {
...
func RandFloatsRange[T Float](sz int, min, max T) []T {
	s := RandFloatsn(sz, max-min)
	for i := range s {
		s[i] += min
	}
	return s
