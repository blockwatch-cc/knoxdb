...
// Benchmark M1
//
// Data           Go Sequence       Indexes        AllIndexes      ChunkView
// -----------------------------------------------------------------------------
...
// An empirical study on synthentic bitsets shows AVX2 indexes (u32)
// outperform step-wise indexes above 25% fill level by up to 8x (at 100%)
// and underperform for sparser bitsets by 2-4x.
//
// [vals/ns]    AllIndexes   Indexes     Diff
// -----------------------------------------------
...
// Ones returns a sequence that ranges over all indexes of bits set
// in the bitset. Its convenient but slow due to function call overhead
// for each returned element. Its ok for tests, but users should prefer
// Indexes() or ChunkView below.
func (s *Bitset) Ones() iter.Seq[int] {
...
// Indexes returns multiple indexes of set bits starting at index i.
// Dst must be allocated and may be filled up to capacity. When no more
// set bits are found, returns a zero length slice and false.
//
// Prefer this method over Ones() and AllIndexes() using a buffer size
...
//
//	idx := make([]int, bitmap.Count())
//	idx, haveAny := bitmap.Indexes(0, idx)
func (s *Bitset) Indexes(last int, dst []int) ([]int, bool) {
	// use full capacity
	nmax := cap(dst)
	dst = dst[:nmax]
	...
	if i&7 > 0 {
		word := s.buf[i>>3] >> (i & 7)
		for word != 0 && n < nmax {
			dst[n] = i + bits.TrailingZeros8(word)
			n++
	...
	for _, word := range arena.FromBytes[uint64](s.buf[i>>3:]) {
		for word != 0 && n < nmax {
			dst[n] = i + bits.TrailingZeros64(word)
			n++
...
// AllIndexes returns all positions for set bits in the bitset. To avoid
// allocation, pass a pre-allocated dst slice with sufficient capacity to
// hold all index positions. Use Count to determine how many ones are in
// the bitset. Uses SIMD for acceleration on hardware with AVX2 support.
func (s *Bitset) AllIndexes(dst []uint32) []uint32 {
	cnt := s.Count()
	if cnt == 0 {
		if dst == nil {
			return nil
	...
	cnt += 7
	if dst == nil || cap(dst) < cnt {
		dst = arena.Alloc[uint32](cnt)[:cnt]
	} else {
		dst = dst[:cnt]
	}
	...
	n := bitsetIndexes(s.buf, s.size, dst)
	return dst[:n]
}

var viewFactory = sync.Pool{
	New: func() any { return new(ChunkView) },
}

// ChunkView is a convenience helper to iterate through bitset indexes
...
func (s *Bitset) View() BitmapViewer {
	it := viewFactory.Get().(*ChunkView)
	it.set = s
	it.last = -1
	return it
}

func (c *ChunkView) Next() ([]int, bool) {
	...
	// regular case
	res, ok := c.set.Indexes(c.last, c.idx[:])
	if ok {
