...
// TODO
// - make pack index compatible with AddTx(), DelTx() which use
//   serialized values (same as WAL) instead of package
// - return bitmap from lookups (must also use bitmap in cond tree)
// - refactor Rebuild() to stream via table
...
type PackIndex struct {
	kind  IndexKind // stored in table metadata
	field *Field    // stored in table metadata
	name  string
	opts  Options
	...
	packidx   *PackHeader // in-memory list of pack and block headers
...
// 		Kind:   i.kind,
// 		Fields: FieldList{i.field},
// 	})
// }

// func (i *PackIndex) UnmarshalJSON(buf []byte) error {
...
func CreatePackIndex(t Table, kind IndexKind, fields FieldList, opts Options) (*PackIndex, error) {
	log.Debugf("Creating %s pack index %s_%s with opts %#v", kind, t.Name(), fields.String(), opts)
	if len(fields) != 1 {
		return nil, fmt.Errorf("pack index supports single field only")
	}
	...
	idx := &PackIndex{
		...
		field:        fields[0],
		opts:         opts,
		table:        t.(*PackTable), // panics on mismatch
		packidx:      NewPackHeader(nil, 0, maxPackSize),
		key:          []byte(fmt.Sprintf("%s_index_%016x", t.Name(), fields.Hash())),
		metakey:      []byte(fmt.Sprintf("%s_index_%016x_meta", t.Name(), fields.Hash())),
		indexValue:   kind.ValueFunc(),
		indexValueAt: kind.ValueAtFunc(),
		indexZeroAt:  kind.ZeroAtFunc(),
	}
	idx.packPool = &sync.Pool{
		New: idx.makePackage,
	}
	idx.stats.IndexName = fmt.Sprintf("%s_%s_index", t.Name(), idx.name)
	...
	err := t.DB().Update(func(tx *Tx) error {
		...
		meta, err := tx.Root().CreateBucketIfNotExists(idx.metakey)
		if err != nil {
			return err
		}
		...
		if err := idx.journal.InitType(PackIndexEntry{}); err != nil {
			return err
		}
		_, err = storePackTx(tx, idx.metakey, idx.journal.Key(), idx.journal, idx.opts.FillLevel)
		if err != nil {
			return err
		}
		...
		if err := idx.tombstone.InitType(PackIndexEntry{}); err != nil {
			return err
		}
		_, err = storePackTx(tx, idx.metakey, idx.tombstone.Key(), idx.tombstone, idx.opts.FillLevel)
		if err != nil {
			return err
		}
		return nil
	...
	log.Debugf("Created %s pack index %s_%s", idx.kind, t.Name(), idx.name)
	return idx, nil
}

func OpenPackIndex(t Table, kind IndexKind, fields FieldList, opts Options) (*PackIndex, error) {
	log.Debugf("Opening %s pack index %s_%s with opts %#v", kind, t.Name(), fields.String(), opts)
	...
	err := t.DB().View(func(tx *Tx) error {
		b := tx.Bucket(idx.metakey)
		if b == nil {
			return ErrNoIndex
		}
		buf := b.Get(optsKey)
		if buf == nil {
			return fmt.Errorf("pack: %s missing index options", idx.name)
		}
		err := json.Unmarshal(buf, &idx.opts)
		if err != nil {
			return err
		}
		if opts.PackSizeLog2 > 0 && opts.PackSizeLog2 != idx.opts.PackSizeLog2 {
			return fmt.Errorf("pack: index %s pack size change not allowed", idx.name)
		}
		idx.opts = idx.opts.Merge(opts)
		maxPackSize := idx.opts.PackSize()
		maxJournalSize := idx.opts.JournalSize()
		idx.stats.JournalTuplesThreshold = int64(maxJournalSize)
		idx.stats.TombstoneTuplesThreshold = int64(maxJournalSize)
		idx.packidx = NewPackHeader(nil, 0, maxPackSize)
		idx.journal, err = loadPackTx(tx, idx.metakey, encodePackKey(journalKey), nil, maxJournalSize)
		if err != nil {
		...
		idx.tombstone, err = loadPackTx(tx, idx.metakey, encodePackKey(tombstoneKey), nil, maxJournalSize)
		if err != nil {
	...
	log.Debugf("Opened %s pack index %s_%s", idx.kind, t.Name(), idx.name)
	return idx, nil
}
...
func (idx *PackIndex) Drop() error {
	...
	return idx.table.DB().Update(func(tx *Tx) error {
		err := tx.Root().DeleteBucket(idx.key)
		if err != nil {
			return err
		}
		return tx.Root().DeleteBucket(idx.metakey)
	})
}

func (idx *PackIndex) loadPackInfo(tx *Tx) error {
	b := tx.Bucket(idx.metakey)
	if b == nil {
		return ErrNoTable
	}
	...
	if bi != nil {
		...
		if err != nil {
			...
			idx.packidx = NewPackHeader(packs, 0, maxPackSize)
...
func (idx *PackIndex) storePackInfo(tx *Tx) error {
	meta := tx.Bucket(idx.metakey)
	if meta == nil {
		return ErrNoTable
	}
...
func (idx *PackIndex) lookupKeys(ctx context.Context, tx *Tx, in []uint64, neg bool) ([]uint64, error) {
	...
	for nextpack := idx.packidx.Len() - 1; nextpack >= 0; nextpack-- {
		// extract min/max values from pack header (this is defined by PackIndexEntry,
...
func (idx *PackIndex) ReindexTx(ctx context.Context, tx *Tx, flushEvery int, ch chan<- float64) error {
	...
	for i := idx.packidx.Len() - 1; i >= 0; i-- {
		...
		if err := deletePackTx(tx, idx.key, key); err != nil {
			return err
		}
	...
	idx.journal.Clear()
	if _, err := storePackTx(tx, idx.metakey, idx.journal.Key(), idx.journal, idx.opts.FillLevel); err != nil {
		return err
	}
	idx.tombstone.Clear()
	if _, err := storePackTx(tx, idx.metakey, idx.tombstone.Key(), idx.tombstone, idx.opts.FillLevel); err != nil {
		return err
	}
	if err := idx.storePackInfo(tx); err != nil {
		return err
	}
...
func (idx *PackIndex) FlushTx(ctx context.Context, tx *Tx) error {
	...
	for {
		...
		for jpos < jlen {
			...
			packmax = max(packmax, pk[jpos])
			globalmax = max(globalmax, packmax)
			nAdd += n
			jpos += n
...
func (idx *PackIndex) splitPack(tx *Tx, pkg *Package) (int, error) {
...
func (idx PackIndex) cachekey(key []byte) string {
	return string(idx.key) + "/" + hex.EncodeToString(key)
}

func (idx *PackIndex) loadSharedPack(tx *Tx, id uint32, touch bool) (*Package, error) {
	// try cache first
	...
	pkg, err := loadPackTx(tx, idx.key, key, idx.newPackage(), idx.opts.PackSize())
	if err != nil {
		return nil, err
	}
	pkg.IncRef()
...
func (idx *PackIndex) loadWritablePack(tx *Tx, id uint32) (*Package, error) {
	// try cache first
	key := encodePackKey(id)
	...
	var err error
	pkg, err = loadPackTx(tx, idx.key, key, pkg, idx.opts.PackSize())
	if err != nil {
		pkg.DecRef()
		return nil, err
	}
...
func (idx *PackIndex) storePack(tx *Tx, pkg *Package) (int, error) {
	key := pkg.Key()
	...
	if pkg.Len() > 0 {
		...
		n, err := storePackTx(tx, idx.key, key, pkg, idx.opts.FillLevel)
		if err != nil {
			return 0, err
		}
...
func (idx *PackIndex) makePackage() interface{} {
...
func (idx *PackIndex) newPackage() *Package {
	return idx.packPool.Get().(*Package)
}

func (idx *PackIndex) releaseSharedPack(pkg *Package) {
	if pkg == nil {
		return
	}
	if pkg.DecRef() == 0 {
